...
@gif for i = 1:Nsteps
    ...
    for e = 1:size(u,2)
        for k = 1:size(u,1)
            l_k = 1
            if rhsu_Diff[k,e] < 0
                l_k = -(u[k,e] + dt*rhsu_ID[k,e]-tol) / (dt*(rhsu_Diff[k,e]))
                l_k = min(l_k, 1)
            end
            l_k = max(l_k,0)
            rhsu1[k,e] = rhsu_ID[k,e] + rhsu_Diff[k,e]*l_k
        end
    end
    # @show L
    utmp = u + dt*rhsu1
    ...
    for e = 1:size(u,2)
        for k = 1:size(u,1)
            l_k = 1
            if rhsu_Diff[k,e] < 0
                l_k = -( 2*u[k,e] + dt*(rhsu_ID[k,e]+ rhsu1[k,e])-tol ) / (dt*rhsu_Diff[k,e] )
                l_k = min(l_k, 1);
            end
            l_k = max(l_k,0)
            rhsu2[k,e] = rhsu_ID[k,e] + rhsu_Diff[k,e]*l_k
        end
    end
    # @show L
    u .+= .5*dt*(rhsu1 + rhsu2)
