...
@gif for i = 1:1000000
    ...
    rhsh_ES1, rhshu_ES1, rhsh_ID1, rhshu_ID1, lambda = rhs(h, hu,ops,vgeo,fgeo,mapP,0)
    dt1 = min(T-t, minimum(w)*J[1]/(8*lambda), dT);
    ...
    rhsh_ES2, rhshu_ES2, rhsh_ID2, rhshu_ID2, lambda = rhs(htmp,hutmp,ops,vgeo,fgeo,mapP,0)
    dt2 = min(T-t, minimum(w)*J[1]/(8*lambda), dT);
    while dt2<dt1
        dt1 = dt2
        # rhsh1, rhshu1, L1 = convex_limiter(rhsh_ES2, rhshu_ES2, rhsh_ID2, rhshu_ID2, h, h, tol, dt1)
        htmp  = h  + dt1*rhsh1
        hutmp = hu + dt1*rhshu1
        hutmp[findall(x->x<2*tol, htmp)] .= 0
        utmp = (htmp, hutmp)
