...
ops = (Qrskew,Qsskew,E,Br,Bs, M_inv, Pf, df)
...
function rhs(u,ops,vgeo,fgeo,nodemaps)
    # unpack args
    Qr,Qs,E,Br,Bs,M_inv,EfTW,df= ops
    rxJ,sxJ,ryJ,syJ,J = vgeo
    nxJ,nyJ,sJ = fgeo
    (mapP,mapB) = nodemaps
    ...
    du = uP-uM
    dudx = rxJ.*(Qr*u) + sxJ.*(Qs*u)
    rhsu = dudx + EfTW * (0.5 * nxJ .* uP)
    # rhsu -= 0.5*abs.(EfTW)*(du.*abs.(nxJ))
    rhsu -= transpose(E)*(df.*du)
...
for i = 1:Nsteps
    ...
    utmp = u + dt*rhsu
    u .+= .5*dt*(rhsu + rhs(utmp,ops,vgeo,fgeo,nodemaps))
